...
<div id="app" v-cloak>
    <el-row :gutter="6">
        <!--        门店列表-->
        <el-col :span="2" style="min-width:300px;" v-loading="storeListLoading">
            <el-card class="store-card">
                <div slot="header">
                    <span>商户列表</span>
                    <el-button style="float: right; padding: 3px 0" type="text" @click="dialogVisible = true">添加商户
                    </el-button>
                </div>
        ...
        <!--        门店分类列表-->
        <el-col :span="2" style="min-width:300px;" v-loading="storeCatLoading">
            <el-card class="store-card">
                <div slot="header">
                    <span>商品分类</span>
                </div>
        ...
        <!--        商品列表-->
        <el-col :span="13" v-loading="storeGoodsLoading" >
            <el-card class="store-card" >
                <div slot="header">
                    <span>商品列表</span>
                </div>
                <el-row style="padding:10px 20px;">
                    <div style="display: inline-block;padding-left:30px;" >
                        导入到商城分类：
                        <el-select v-model="store_cat_id" placeholder="导入到分类">
                            <el-option
                                    v-for="item in store_cat_list"
                                    :key="item.value"
                                    :label="item.label"
                                    :value="item.value">
                            </el-option>
                    ...
                    <el-button type="primary" size="small" @click="handlCopyGoods()">批量导入</el-button>

                </el-row>
                <el-row>
                    <div style="padding:10px 20px;color:red;">一次性不要导入太多，图片下载时间过长</div>
                </el-row>
                <div class="store-content" :style="{height: height-100 + 'px',overflow:'auto'}" >
                    <el-table :data="goodsList" border style="width: 100%;margin-bottom: 10px"
                              @selection-change="handleSelectionChange"
                    >
                        <el-table-column prop="goods_id" label="ID" width="80" type="selection"></el-table-column>
                        ...
                        <el-table-column
                                label="导入状态"
                                :filters="[{ text: '已导入', value: '1' }, { text: '未导入', value: '0' }]"
                                :filter-method="filterTag"
                        >
                            <template slot-scope="scope">
                                <div flex="box:first">
                                    <el-tag  v-if="scope.row.is_copy == 1" type="success">已导入</el-tag>
                                    <el-tag v-else>未导入</el-tag>
                                </div>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column label="操作">
                            <template slot-scope="scope">
                                <el-button @click="handlRowCopyGoods(scope.row.goods_id)" type="text" circle size="mini">
                                    导入
                                </el-button>
                ...
                <el-row style="height: 0px;line-height: 4px;text-align: center;">共{{goodsList.length ? goodsList.length:0}}个商品</el-row>
            </el-card>
        </el-col>
    </el-row>
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                height: document.documentElement.clientHeight - 100, // 表的高度
                //门店
                dialogVisible: false,
        ...
        methods: {
            ...
            getStoreGoodsList() {
                ...
                request({
                    params: {
                        r: 'plugin/copy/mall/index/goods-list',
                        store_id:self.store_id,
                        cat_id:self.cat_id,
                        status:self.status
                    },
            ...
            copyGoods(ids){
                ...
                request({
                    params: {
                        r: 'plugin/copy/mall/index/copy-goods',
                    },
                    method: 'post',
                    data: {
                        store_id:self.store_id,
                        cat_id:self.store_cat_id,
                        ids:ids.join(","),
                    }
                }).then(e => {
                    self.storeGoodsLoading = false
                    if(e.data.code ==1 ){
                        ...
                        this.$alert('导入失败'+e.data.msg, '提示', {
                            confirmButtonText: '确定'
                        });

                    }else{
                        this.$alert('导入成功,'+e.data.msg, '提示', {
                            confirmButtonText: '确定',
                            callback: action => {
                                self.getStoreGoodsList()
                            }
                        });
                    }
