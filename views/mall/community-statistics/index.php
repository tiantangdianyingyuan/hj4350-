...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)">社区团购</app-header>
        </div>
        ...
        <div class="table-body" style="padding: 20px">
            <el-table ref="community" @sort-change="changeSort" v-loading="loading" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                ...
                <el-table-column prop="user_num" label="拼团人数" :label-class-name="prop == 'user_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                </el-table-column>
                <el-table-column prop="pay_user_num" label="支付人数" :label-class-name="prop == 'pay_user_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="pay_goods_num" label="支付件数" :label-class-name="prop == 'pay_goods_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="pay_price" label="支付金额" :label-class-name="prop == 'pay_price' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column label="状态">
                    <template slot-scope="scope">
                        <span v-if="scope.row.activity_status == '未开始'" style="color: #FFA360">{{scope.row.activity_status}}</span>
                        <span v-if="scope.row.activity_status == '进行中'" style="color: #4BC282">{{scope.row.activity_status}}</span>
                        <span v-if="scope.row.activity_status == '已结束'" style="color: #FF8585">{{scope.row.activity_status}}</span>
                    </template>
                </el-table-column>
            </el-table>
